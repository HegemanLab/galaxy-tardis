...
- [Motivation: To Administer a Local Galaxy with Minimal Stress](#motivation-administering-a-local-galaxy-with-minimal-stress)
...
### Motivation: To Administer a Local Galaxy with Minimal Stress
...
- Ensure that `tags-for-tardis_envar-to-source.sh` exists
    ...
    - or by copying `restore_example/globals.example` to `restore_example/globals.my_instance`, customizing it, and:<br />
      `pushd restore_example; bash setup_env globals.my_instance; popd`
        - More information about this approach is presented in ["An Example of a Galaxy backed up by S3"](#an-example-of-a-galaxy-backed-up-by-s3)
- Set the `TARDIS` environment variable and invoke the TARDIS, e.g. (in bash):<br />
  `source tardis_envar.sh # set the TARDIS environment variable`<br />
  `$TARDIS help`
...
- How to instantiate a Galaxy 19.01 instance on `docker-compose` (under Usernetes).
...
- `restore_example/globals.my_instance` (named as you like)
    - Copy this from `restore_example/globals.example`
    - Adjust the many variables to meet your needs.
    - Write the environment files used by the example scripts:
        - `bash setup_env globals.my_instance`
...
- `env-for-DEMO`, which is used by the `restore_example/DEMO` script
...
---
name: restore_example---run-modes

### `restore_example`--Run Modes

- To instantiate a fresh Galaxy:
```bash
bash -c "export DEMO fresh"
```
- To instantiate restore a Galaxy from an S3-compatible bucket:
```bash
bash -c "export DEMO restore"
```
- To instantiate run the existing Galaxy:
```bash
bash -c "export DEMO run"
```

The following slides describe what the DEMO script does in each of these cases.
...
When you invoke `bash DEMO fresh`, the following occur:
...
When you invoke `bash DEMO restore`, the following occur:
- Directories are created as specified in config files, if necessary.
- Galaxy is initialized, as is the database:
```bash
    ./compose_start.sh --init-only --init-db
...
When you invoke `bash DEMO run`, the following occur:
has already been initialized), then, when you `bash DEMO`:
- `compose_start.sh` is invoked without options to launch Galaxy.
  - If `screen` is installed, Galaxy will be launched within it.

If you are very lucky and you have not yet initialized Galaxy,
the rudimentary error checking for this case will warn you;
don't rely on it. :)
